...
function displayText(text: string): string {
  return text
    ...
    // Strip partial (unclosed) tags — prevents raw JSON leaking during streaming
    .replace(/<policyProposal[\s\S]*/g, '')
    .replace(/<policyProposalRepair[\s\S]*/g, '')
    .replace(/<policyProposalError[\s\S]*/g, '')
    .trim()
}
...
export function AiPolicyAssistant({ categories, ruleItems, vendors = [] }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Streaming text */}
      {streaming && (
        <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 px-5 py-4 space-y-3">
          <div className="flex items-center gap-2">
            <svg className="animate-spin w-3.5 h-3.5 text-blue-400 shrink-0" viewBox="0 0 24 24" fill="none">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
            </svg>
            <span className="text-xs font-semibold text-blue-400">Drafting policy proposal…</span>
          </div>
