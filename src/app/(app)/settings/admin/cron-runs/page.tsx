    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500">
          Weekly AI-powered compliance review — every Monday at 3 AM UTC.
          Claude checks each regulation for factual changes and auto-updates the database.
        </p>
      </div>
      ...
      {runs.length === 0 ? (
        <div className="py-16 text-center rounded-xl border border-zinc-800">
          ...
          <p className="text-xs text-zinc-600 mt-1">The first run will appear here after the cron executes on Monday.</p>
        </div>
      ) : (
            ...
            <div key={run.id} className="rounded-xl border border-zinc-800 bg-zinc-900/40 overflow-hidden">
              ...
              {run.status === 'completed' && run.regs_updated === 0 && run.errors?.length === 0 && (
                <div className="border-t border-zinc-800 px-5 py-3">
                  <p className="text-xs text-zinc-600">All regulations verified — no factual changes detected.</p>
                </div>
              )}
            </div>
          ))}
