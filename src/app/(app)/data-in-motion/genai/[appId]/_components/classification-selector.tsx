...
export function ClassificationSelector({ appId, orgId, currentClassification }: Props) {
  ...
  const COLOR_MAP: Record<string, string> = {
    ...
    zinc:   'border-zinc-600 bg-zinc-800/60 text-zinc-300',
  }
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2">
        ...
        {isPending && <Loader2 className="h-4 w-4 animate-spin text-zinc-500 self-center" />}
        {saved && <span className="text-xs text-green-400 self-center">Saved</span>}
      </div>
      {!orgId && (
        <p className="text-xs text-zinc-600 mt-2">Sign in to set your organisation's classification.</p>
      )}
    </div>
  )
}
