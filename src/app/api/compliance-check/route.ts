...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Review all regulations in parallel (batched to avoid overwhelming the API)
    const BATCH = 5
      ...
      await Promise.all(batch.map(async reg => {
        try {
          ...
          if (!result.changed || !result.updates) {
            // No change — just bump last_verified_at
            await supabase
              .from('compliance_regulations')
          ...
          // Update regulation top-level fields
          ...
          // Log to verification log (system update — no org_id, use sentinel)
